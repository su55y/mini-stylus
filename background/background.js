...
addAPI(
  /** @namespace API */ {
    ...
    async openEditor(params) {
      ...
      const wndBase =
        wnd && prefs.get('openEditInWindow.popup') ? { type: 'popup' } : {}
        ...
        try {
          tab =
            ...
            (await openURL({
              ...
              currentWindow: null,
              newWindow:
                wnd && Object.assign({}, wndBase, !ffBug && !retry && wndPos),
            }))
          if (ffBug && !retry)
            await browser.windows.update(tab.windowId, wndPos)
          return tab
        } catch (e) {}
      }
    },
    ...
    async openManage(opts = {}) {
      ...
      if (!tab) {
        ...
        await msg
          .sendTab(tab.id, { method: 'pushState', url: setUrlParams(tab.url) })
      ...
      function onTabReady(tab) {
        return new Promise((resolve, reject) =>
          setTimeout(function ping(numTries = 10, delay = 100) {
            msg
              .sendTab(tab.id, { method: 'ping' })
              .catch(() => false)
              .then((pong) =>
                pong
                  ? resolve(tab)
                  : (numTries &&
                      setTimeout(ping, delay, numTries - 1, delay * 1.5)) ||
                    reject('timeout')
              )
          })
        )
    ...
    for (const dbName of ['drafts', prefs.STORAGE_KEY]) {
      try {
        indexedDB.open(dbName).onsuccess = async (e) => {
          ...
          const ta =
            idb.objectStoreNames[0] === 'data' && idb.transaction(['data'])
...
Promise.all([
  ...
  FIREFOX && require(['/background/style-via-api']),
  FIREFOX &&
    (browser.commands || {}).update &&
    require(['/background/browser-cmd-hotkeys']),
  !FIREFOX && require(['/background/content-scripts']),
  chrome.contextMenus && require(['/background/context-menus']),
]).then(() => {
  bgReady._resolveAll()
  msg.ready = true
