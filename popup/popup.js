...
const $entry = (styleOrId) =>
  $(`#${ENTRY_ID_PREFIX_RAW}${styleOrId.id || styleOrId}`)
...
function onRuntimeMessage(msg) {
  ...
  switch (msg.method) {
    case 'styleAdded':
    case 'styleUpdated':
      if (msg.reason === 'editPreview' || msg.reason === 'editPreviewEnd')
        return
...
function setPopupWidth(_key, width) {
  document.body.style.width = clamp(width, 200, 800) + 'px'
}
...
async function initPopup(frames) {
  ...
  if (frames.length > 1 && $('.match .match:not(.dupe)')) {
    $('#write-style').append(
      Object.assign(t.template.writeForFrames, {
        onclick() {
          this.remove()
  ...
  for (let retryCountdown = 10; retryCountdown-- > 0; ) {
    ...
    if (
      await msg
        .sendTab(tab.id, { method: 'ping' }, { frameId: 0 })
        .catch(() => {})
    ) {
      return
    }
...
function initUnreachable(isStore) {
  ...
  if (!FIREFOX) {
    ...
    const note = [
      ...
      isStore &&
        t(FIREFOX >= 59 ? 'unreachableAMOHint' : 'unreachableMozSiteHintOldFF'),
    ...
    const renderLine = (line) =>
      $create('p', line.split(/(<.*?>)/).map(renderToken))
...
function createWriterElement(frame, index) {
  ...
  Object.assign(urlLink, {
    ...
    onclick: (elFor.onclick = (e) =>
      Events.openEditor(e, { 'url-prefix': url })),
  })
  if (prefs.get('popup.breadcrumbs')) {
    urlLink.onmouseenter = urlLink.onfocus = () =>
      urlLink.parentNode.classList.add('url()')
    urlLink.onmouseleave = urlLink.onblur = () =>
      urlLink.parentNode.classList.remove('url()')
  }
  ...
  const child = $create({
    tag: 'span',
    className: `match${isDupe ? ' dupe' : ''}${
      isAboutBlank ? ' about-blank' : ''
    }`,
    dataset: { frameId },
    appendChild: targets,
  })
...
function showStyles(frameResults) {
  ...
  const zebra =
    $('.entry:last-child:nth-child(odd)') && !$('.styles-last')
      ? 'reverse-zebra'
      : 'zebra'
...
function createStyleElement(style) {
  ...
  if (!entry) {
    ...
    if (!style.usercssData) {
      if (style.updateUrl && style.updateUrl.includes('?') && style.url) {
        ...
        $('use', config).attributes['xlink:href'].nodeValue =
          '#svg-icon-config-uso'
      } else {
    ...
    $('.exclude-by-domain-checkbox', entry).onchange = (e) =>
      Events.toggleExclude(e, 'domain')
    $('.exclude-by-url-checkbox', entry).onchange = (e) =>
      Events.toggleExclude(e, 'url')
  }
  ...
  setTimeout(() => {
    styleName.title = entry.styleMeta.sloppy
      ...
      : entry.styleMeta.excludedScheme
      ? t(`styleNotAppliedScheme${capitalize(entry.styleMeta.preferScheme)}`)
  ...
  entry.classList.toggle(
    'not-applied',
    style.excluded || style.sloppy || style.excludedScheme
  )
  entry.classList.toggle('regexp-partial', style.sloppy)

  $('.exclude-by-domain-checkbox', entry).checked = Events.isStyleExcluded(
    style,
    'domain'
  )
  $('.exclude-by-url-checkbox', entry).checked = Events.isStyleExcluded(
    style,
    'url'
  )
  ...
  if (frameUrl) {
    ...
    const frameEl =
      $(sel, entry) || styleName.insertBefore($create(sel), styleName.lastChild)
